...
export default function DoctorSignUp() {
  ...
  const registerAdministrator = async (e: any) => {
    (e as PointerEvent).preventDefault();

    const res = await postWithNoAuth('/employee/register', {
  ...
  useEffect(() => {
    if (auth) nav('/hospital/doctors');
  }, [auth]);

  return (
    <div className="auth flex">
      <main className="auth__main">
        <form className="auth__main__form" onSubmit={registerAdministrator}>
          <div className="auth__main__form__title">
            <h1><span>MY</span>PHARMACIST</h1>
            <p>Hospital sign up</p>
          </div>
          <div className="auth__main__form__body">
            <p className="error hide" id="auth-error">Erro</p>

            <div className="input">
              <input type="text" id="fullname" placeholder="Hospital name" />
            </div>
            <div className="input">
